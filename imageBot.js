...
var T = new Twit({
  consumer_key:         'DvVWCceRg5CxegF2Vhn2YV3NA',
  consumer_secret:      'jFd4of7Ne9oSYux7IDYMHp6TbPaxkorIs4eEOhXCRvWqbRBoph',
  access_token:         '797518880025563139-ovI35N8kLalHJYwB29Tenv5bkuQK8JV',
  access_token_secret:  'rruFh45tAUftHF3o7l0itnB6GsO03FkLpNwFhiyxLBGNb',
...
function tweetReply(tweet_status_id,reply_tweet,imageFileName){
    ...
    if (!test_only) {
	    ...
	    console.log('Media Uploaded')
            ...
            var altText = "Image automatically painted in a famous style using neural nets #deeplearning"
...
function tweetProcess(tweet_status_id,reply_tweet,imageFileName){
    ...
    PythonShell.run('network.py', { args: [imageFileName,fromStyleFileName, styleFileName ]}, function(err, results){
	 ...
	 console.log(err);
          ...
          tweetReply(tweet_status_id,reply_tweet,styleFileName+"_at_iteration_1.png");
    });
}
...
function searchCallback_old(err, data, response) {
  ...
  for (index in tweets) {
    ...
    imageStream.on('close', function(inputImage) {
        ...
        var reply_tweet  = 'Hi @' + tweetMsg.user.screen_name + ' how\'s this? Style transfered by neural nets #DEEPLEARNING #STYLETRANSFER';
...
function processImageTweet (tweetMsg) {
   ...
   var reply_tweet  = 'Hi @' + tweetMsg.user.screen_name + ' how\'s this? Stylized by neural nets #DEEPLEARNING #STYLIZE ' + textStr;
...
// Create a store
var tweetstore = nStore.new('data/tweets.db', function () {
  // It's loaded now
  console.log("tweets cache loaded");
...
function searchCallback(err, data, response) {
  ...
  for (index in tweets) {
    ...
    tweetstore.get(tweets[index].id, function(err, doc, key){
      if(err){

    tweetstore.save(tweets[index].id, {replied: true}, function(err){
      if(err){
        console.log("couldn't save tweet id");
      }else {
        processImageTweet(tweets[index])
      }
    ...
    if (replyTo === 'StylizeBot') {
        ...
        if (from === 'StylizeBot') {
            ...
            if (eventMsg.entities.media)
                ...
                var reply_tweet = '@' + from + ' thank you for tweeting me. Tweet me an image and I will stylize it like a famous painting.'
...
cron.scheduleJob(rule, function(){
  /*tweetstore.get(1234, function(err, doc, key){

  if(err){
  tweetstore.save(1234,{replied: true},function(err){
    if(err){
  ...
  T.get('search/tweets', { q:searchString ,  result_type: 'mixed',since: datestring(),  count: 10, exclude:'replies', exclude:'retweets', filter:'twimg' }, searchCallback)
    console.log(new Date(), 'Searching Tweets Again.');
});
